...
c2d=$script_path/../cocos2d-x-lite
c3b=$script_path/../cocos2d-x-lite-external

if [ ! -d ${c2d}  ]; then
    echo "-- cocos2d-x-lite folder not found!"
    exit -1
fi

if [ ! -d ${c3b}  ]; then
    echo "-- cocos2d-x-lite-external folder not found!"
    exit -1
fi
...
echo "-- cocos2d-x-lite-external version: ${branch}"

# cd ${c2d}
# git clean -fdx external
# cd ..
cd ${c3b}
echo "-- update cocos2d-x-lite-external repo ... "
git fetch --all
